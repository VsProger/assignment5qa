...
func TestSignUp(t *testing.T) {
	...
	const (
		validUsername = "bauka"
		validEmail    = "bauka1@gmail.com"
		validPassword = "Qwerty1234!"
	)
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCode:      http.StatusSeeOther, // 303
		},
		{
			name:          "Blank username",
			username:      "",
			...
			password:      validPassword,
			passwordAgain: validPassword,
			wantCode:      http.StatusUnprocessableEntity, // 422
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Если обработчик ожидает два значения для поля "password" (например, для подтверждения),
			// то оставляем так, как есть
...
func TestUserLoginPost(t *testing.T) {
	...
	// Регистрируем пользователя для успешного логина
	const (
		validName     = "bauka"
		validEmail    = "bauka@gmail.com" // используем один email для регистрации и логина
		validPassword = "Qwerty1234!"
	)

	// Выполняем регистрацию
	signupForm := url.Values{}
	signupForm.Add("name", validName)
	signupForm.Add("email", validEmail)
	signupForm.Add("password", validPassword)
	signupForm.Add("password", validPassword)
	signupCode, _, _ := ts.postForm(t, "/signup", signupForm)
	if signupCode != http.StatusSeeOther {
		t.Fatalf("Signup failed for login test: got %d, want %d", signupCode, http.StatusSeeOther)
	}
	...
	tests := []struct {
		...
		},
		{
			...
			email:    validEmail,
			password: validPassword,
			// Если успешный логин должен перенаправлять, ожидаем 303:
			wantCode: http.StatusSeeOther, // 303
			// Если же спецификация говорит, что должен быть 202, оставьте 202,
			// но тогда необходимо исправить логику в обработчике /login.
		},
	}
